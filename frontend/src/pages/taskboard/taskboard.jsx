...
function readStoredPresets() {
  try {
    const value = JSON.parse(
      localStorage.getItem("nowlTaskBoardPresets") || "[]",
    );
...
function TaskBoardPage() {
  ...
  const loadCalendarTasks = useCallback(async () => {
    const data = await apiRequest("/api/calendar/items?type=task");
  ...
  const canEdit =
    activeBoard?.currentUserRole === "owner" ||
  ...
  const boardMembers = useMemo(() => {
    ...
    return [owner, ...Object.values(activeBoard.members || {})];
  }, [activeBoard]);
  ...
  async function handleRemoveBoardMember(member) {
    ...
    try {
      await apiRequest(
        `/api/task-boards/${activeBoard.id}/members/${member.uid}`,
        {
          method: "DELETE",
        },
      );
  ...
  async function handleDeleteCard(card) {
    ...
    try {
      await apiRequest(`/api/task-boards/${activeBoard.id}/cards/${card.id}`, {
        method: "DELETE",
      });
  ...
  function handleDragStart(event, item) {
    ...
    setDraggedItem(item);
    event.dataTransfer.effectAllowed = item.kind === "card" ? "move" : "copy";
    event.dataTransfer.setData("text/plain", JSON.stringify(item));
  }

  function handleColumnDragStart(event, columnId) {
    event.stopPropagation();
    const item = { kind: "column", id: columnId };
    setDraggedItem(item);
    event.dataTransfer.effectAllowed = "move";
    event.dataTransfer.setData("text/plain", JSON.stringify(item));
  }

  async function moveColumn(columnId, targetColumnId, placeAfter = false) {
    if (!activeBoard || !isOwner || saving || columnId === targetColumnId)
      return;

    const previousColumns = activeBoard.columns;
    const movedColumn = previousColumns.find(
      (column) => column.id === columnId,
    );
    if (!movedColumn) return;

    const remainingColumns = previousColumns.filter(
      (column) => column.id !== columnId,
    );
    ...
    nextColumns.splice(insertAt, 0, movedColumn);
    const columns = nextColumns.map((column, index) => ({
      ...column,
      position: index,
    }));

    setActiveBoard((current) => ({ ...current, columns }));
    setBoards((current) =>
      current.map((board) =>
    ...
    setSaving(true);
    try {
      const data = await apiRequest(`/api/task-boards/${activeBoard.id}`, {
        method: "PATCH",
      ...
      setBoards((current) =>
        current.map((board) =>
          board.id === data.board.id ? { ...board, ...data.board } : board,
        ),
      );
      setNotice("Column order updated.");
    } catch (error) {
      setActiveBoard((current) => ({
        ...current,
  ...
  function handleColumnDrop(event, columnId) {
    ...
    if (item.kind === "column") {
      ...
      moveColumn(item.id, columnId, placeAfter);
        ...
        <div
          ...
          className={`task-board-sidebar__section ${
            isMobileBoardMenuOpen
              ? "task-board-sidebar__section--mobile-open"
              : ""
          }`}
        >
          ...
          <div className="task-board-sidebar__boards">
            {boards.map((board) => (
              ...
              >
                ...
                <span className="task-board-sidebar__label">{board.name}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="task-board-shell">
        ...
        {pageStatus !== "loading" && boards.length === 0 && (
          <section className="task-board-empty">
            ...
            <p>
              Each board starts with Backlog, In Progress, Completed, and
              Reminders columns.
            </p>
            <button
              type="button"
            ...
            <div className="task-board-workspace">
              <aside className="task-card-library">
                ...
                <div className="task-card-library__tabs">
                  <button
                    type="button"
                    className={libraryTab === "calendar" ? "is-active" : ""}
                  ...
                  <button
                    type="button"
                    className={libraryTab === "presets" ? "is-active" : ""}
                    ...
                    filteredPresets.map((preset) => (
                      ...
                      >
                        <div className="library-card__topline">
                          <span className="library-card__type">Preset</span>
                          <button
                            type="button"
                            onClick={() =>
                              setPresets((current) =>
                  ...
                  return (
                    <article
                      className={`task-board-column${
                        draggedItem?.kind === "column" &&
                        draggedItem.id === column.id
                          ? " task-board-column--dragging"
                          : ""
                      }${
                        columnDropTarget === column.id
                          ? " task-board-column--drop-target"
                          : ""
                      }`}
                      key={column.id}
                      onDragOver={(event) => {
                      ...
                      <div className="task-board-column__cards">
                        {columnCards.map((card) => (
                          <article
                            ...
                            onClick={() => canEdit && openEditCard(card)}
                          >
                            ...
                            <footer className="task-board-card__footer">
                              <div>
                                {card.dueAt && (
                                  ...
                                  >
                                    ◷ {formatDate(card.dueAt, card.allDay)}
                                  </span>
                                )}
                                {card.calendarItemId && (
                                  <span title="Imported from calendar">▣</span>
                                )}
                              </div>
                                    ...
                                    .map((assignee) => (
                                      ...
                                      >
                                        {assignee.profilePicLink ? (
                                          <img
                                            src={assignee.profilePicLink}
                                            alt=""
                                          />
                                        ) : (
                                          avatarText(assignee.displayName)
                                        )}
                                      </span>
                                    ))}
                                </div>
        ...
        <div className="task-board-modal-backdrop">
          <form className="task-board-modal" onSubmit={handleCreateBoard}>
            <header>
              ...
              <button type="button" onClick={() => setBoardModalOpen(false)}>
                ×
              </button>
            </header>
            ...
            <fieldset className="task-board-member-fieldset">
              ...
              />
              <small>
                Members can be added now or later. Editors can change cards;
                ...
                <div className="task-board-selected-members">
                  {boardForm.members.map((member) => (
                    <div key={member.uid}>
                      <span className="task-board-avatar">
                        {member.profilePicLink ? (
                          <img src={member.profilePicLink} alt="" />
                        ) : (
                          avatarText(member.displayName)
                        )}
                      </span>
                      <span className="task-board-member-name">
                        {member.displayName}
                      ...
                      <button
                        type="button"
                        onClick={() => togglePendingBoardMember(member)}
              ...
              <div className="task-board-user-results">
                ...
                {memberSearchStatus === "success" &&
                  memberSearchResults.map((user) => {
                    const selected = boardForm.members.some(
                    ...
                    return (
                      <button
                        ...
                        onClick={() => togglePendingBoardMember(user)}
                      >
                        <span className="task-board-avatar">
                          {user.profilePicLink ? (
                            <img src={user.profilePicLink} alt="" />
                          ) : (
            ...
            <div className="task-board-user-results">
              ...
              {memberSearchStatus === "success" &&
                memberSearchResults
                  .filter(
                    (user) =>
                      !boardMembers.some((member) => member.uid === user.uid),
                  )
                  .map((user) => (
                    <button
                      type="button"
                      key={user.uid}
                      onClick={() => handleAddExistingBoardMember(user)}
                      disabled={saving}
                    >
                      <span className="task-board-avatar">
                        {user.profilePicLink ? (
              ...
              {boardMembers.map((member) => (
                <div key={member.uid}>
                  ...
                  <span className="task-board-member-name">
                    {member.displayName}
                    <small>
                      {member.role === "owner" ? "Board owner" : member.email}
                    </small>
                  </span>
                  {member.role === "owner" ? (
                    <strong>Owner</strong>
                    ...
                    <>
                      <select
                        value={member.role}
                        onChange={(event) =>
                          handleChangeMemberRole(member, event.target.value)
                        }
                        disabled={saving}
                      >
                        <option value="editor">Editor</option>
                      ...
                      <button
                        type="button"
                        className="task-board-member-remove"
            ...
            <div className="task-board-form-grid">
              ...
              <fieldset className="task-board-form-grid__full">
                ...
                <div className="task-board-assignee-picker">
                  {boardMembers.map((member) => {
                    const checked = cardForm.assigneeIds.includes(member.uid);
                    return (
                          ...
                          onChange={() =>
                            setCardForm((current) => ({
                              ...
                              assigneeIds: checked
                                ...
                                : [...current.assigneeIds, member.uid],
                            }))
                          }
                        />
                        <span className="task-board-avatar">
        ...
        <div className="task-board-modal-backdrop">
          <form className="task-board-modal" onSubmit={handleSavePreset}>
            ...
            <p className="task-board-modal__note">
              Presets are stored in this browser. Your current backend does not
              yet include shared preset routes.
            </p>
